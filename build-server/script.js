...
const KAFKA_TOPIC = process.env.KAFKA_TOPIC || 'build-logs';
    ...
    try {
        ...
        function walkDir(currentPath) {
            ...
            for (const file of files) {
                if (file === '.git' || file === 'node_modules' || file === '.env') continue;
...
async function init() {
    ...
    const runUpload = async () => {
        ...
        if (fs.existsSync(distFolderPath)) {
            ...
            folderToUpload = projectPath;
            await publishLog('No compiled output folder found. Deploying root repository files.', 'INFO');
        }
    ...
    if (!buildCommand) {
        await publishLog('No build command required (static project). Proceeding to deployment.', 'INFO');
        await runUpload();
        return;
    }
