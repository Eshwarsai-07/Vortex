...
const Deploy = () => {
    ...
    useEffect(() => {
        const fetchBranches = async () => {
            try {
                const response = await api.get(`${import.meta.env.VITE_API_URL || 'http://13.235.26.67'}/api/github/branches/${username}/${repo}`);
        ...
        const saveDeploymentData = async () => {
            if (isDeployed && deploymentId) {
                try {
                    ...
                    await api.post(`${import.meta.env.VITE_API_URL || 'http://13.235.26.67'}/api/deploy/create`, {
                        deploymentId,
                        repoName: repo,
    ...
    const handleDeploy = async () => {
        ...
        try {
            await api.post(`${import.meta.env.VITE_API_URL || 'http://13.235.26.67'}/api/deploy/start`, {
                repo,
                branch: selectedBranch,
                username,
                deploymentId,
            ...
            const pollLogs = async () => {
                ...
                try {
                    const response = await api.get(`${import.meta.env.VITE_API_URL || 'http://13.235.26.67'}/api/logs/${deploymentId}`, {
                        signal: abortControllerRef.current.signal,
                        params: {
                ...
                <div className="bg-[#111] border border-gray-800 rounded-3xl p-6 shadow-2xl h-[600px] flex flex-col">
                    ...
                    <div className="mt-5 text-center flex justify-center gap-4">
                        {isDeployed ? (
                            <>
                                <a
                                    href={`https://eshwar-vortex-storage.s3.eu-north-1.amazonaws.com/__outputs/${deploymentId}/index.html`}
                                    target="_blank"
